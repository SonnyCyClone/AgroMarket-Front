...
import { Component, OnInit, OnChanges, SimpleChanges, Input, Output, EventEmitter, signal, inject } from '@angular/core';
...
export class EditProductModalComponent implements OnInit, OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes['product'] && this.product && this.editForm) {
      this.initializeForm();
    }
  }
    ...
    if (this.editForm.valid) {
      ...
      try {
        ...
        if (this.selectedImageFile()) {
          ...
          // Mantener imagen existente o undefined si no hay
          if (formValues.imagenUrl && formValues.imagenUrl.trim() !== '') {
            formData.append('ImagenUrl', formValues.imagenUrl);
          }
          // Si no hay imagen, no agregamos el campo para que el API lo maneje como undefined
        }

        // Realizar llamada PUT al API
        const response = await firstValueFrom(this.http.put(`${this.API_BASE_PRODUCT}/api/Producto`, formData));

        // Crear producto actualizado para retornar
        const updatedProduct: Product = {
          ...this.product,
          ...formValues
        };

        // Si hay nueva imagen, actualizar la URL
        if (this.selectedImageFile()) {
          // El API debería retornar la nueva URL de la imagen
          // Por ahora usamos el preview local
          updatedProduct.imagenUrl = this.imagePreview();
        }
